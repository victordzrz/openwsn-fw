...
void openserial_messageAppendBuffer(uint8_t * buffer,uint8_t lenght){
  memcpy(&(openserial_message[openserial_messagePointer]),buffer,lenght);
  openserial_messagePointer+=lenght;
}

void openserial_messageAppend(uint8_t value){
  openserial_message[openserial_messagePointer++]=value;
}

void openserial_messageReset(){
  openserial_messagePointer=0;
}

void openserial_messageFlush(){
  openserial_printMessage(openserial_message,openserial_messagePointer);
  openserial_messagePointer=0;
}
